...
def clear_cache():
    ...
    if click.confirm(
        ...
        pytest_cache_dir = os.path.join(
            os.getenv("WORKING_DIR", ""), "app/modules/.pytest_cache"
        )
        ...
        if os.path.exists(build_dir):
            ...
            click.echo(
                click.style(
                    "No cache or build directory found. Nothing to clear.", fg="yellow"
                )
            )
        ...
        for dir in pycache_dirs:
            try:
                ...
                click.echo(
                    click.style(
                        f"Failed to clear __pycache__ directory {dir}: {e}", fg="red"
                    )
                )
