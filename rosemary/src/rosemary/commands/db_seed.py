...
def get_module_seeders(module_path, specific_module=None):
    ...
    # Sort seeders by priority. splent_framework's BaseSeeder has no `priority`
    # attribute, so default to 10 (the value the old core BaseSeeder used) to
    # preserve the historical seeding order without touching every seeder.
    seeders.sort(key=lambda seeder: getattr(seeder, "priority", 10))
